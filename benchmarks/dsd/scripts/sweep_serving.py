...
class BenchEngine:
    def __init__(self, setting: BenchSetting) -> None:
        ...
        # launch backend
        cmd = f"vllm serve {setting.model} "
        cmd += f" --tensor-parallel-size {setting.tp} "
        cmd += f" --disable-log-requests"
        # cmd += f" --max-model-len 40960"
        cmd += f" --port {setting.port}"
        cmd += f" --enable-chunked-prefill=False"
        cmd += f" --acceptance-rate {args.acceptance_rate}" \
            if args.acceptance_rate is not None else ""
        cmd += f" --dsd" if args.dsd is True else ""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default="/work/js_park/ShareGPT_V3_unfiltered_cleaned_split.json",
    )

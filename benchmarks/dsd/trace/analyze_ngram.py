...
match_9_filename = "/data/lily/vllm-dsd-osdi/benchmarks/dsd/trace/64_k3_0.9.json"
match_5_filename = "/data/lily/vllm-dsd-osdi/benchmarks/dsd/trace/64_k3_0.5.json"
...
def get_request_latency(request_traces):
    ...
    for trace in request_traces:
        if trace['end_us'] is not None and trace['start_us'] is not None:
            request_latencies[trace['tid']] = (trace['end_us'] - trace['start_us']) / 1e6
...
def analyze(filename):
    ...
    avg_req_exec_time = sum(get_req_exec_times(step_traces).values()) / len(request_traces)

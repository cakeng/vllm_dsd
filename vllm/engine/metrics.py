...
class BatchedFileLogger(StatLoggerBase):
    ...
    def stats_to_str(self, stats: Stats) -> str:
        ...
        if stats.spec_decode_metrics:
            ...
            output += f"num_batch: {sdMetric.num_batch}\n"
            output += f"num_batched_tokens: {sdMetric.num_batched_tokens}\n"
            output += f"num_proposed_tokens: {sdMetric.num_proposed_tokens}\n"
            output += f"num_accepted_tokens: {sdMetric.num_accepted_tokens}\n"
            output += f"timestamp: {sdMetric.timestamp}\n"
        else:
            output += f"draft_acceptance_rate: None\n"
            output += f"system_efficiency: None\n"
            output += f"draft_tokens: None\n"
            output += f"emitted_tokens: None\n"
            output += f"accepted_tokens: None\n"
            output += f"num_spec_tokens: None\n"
            output += f"num_batch: None\n"
            output += f"num_batched_tokens: None\n"
            output += f"num_proposed_tokens: None\n"
            output += f"num_accepted_tokens: None\n"
            output += f"timestamp: None\n"
        output += "\n"
        return output
